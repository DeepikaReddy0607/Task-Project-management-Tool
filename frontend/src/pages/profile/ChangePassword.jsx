...
function ChangePassword() {
  ...
  const { register, handleSubmit, getValues, watch, formState: { errors } } = useForm({ defaultValues: { currentPassword: "", newPassword: "", confirmPassword: "" } });
  const newPassword = watch("newPassword");
  const confirmPassword = watch("confirmPassword");
  const strengthScore = [newPassword.length >= 6, newPassword.length >= 10, /[A-Z]/.test(newPassword), /[0-9]/.test(newPassword), /[^A-Za-z0-9]/.test(newPassword)].filter(Boolean).length;
  const strength = !newPassword ? null : strengthScore <= 2 ? { label: "Weak", color: "bg-[var(--color-danger)]", hint: "Add length, a number, or a symbol." } : strengthScore <= 3 ? { label: "Fair", color: "bg-[var(--color-sun)]", hint: "A little more variety will make it stronger." } : { label: "Strong", color: "bg-[var(--color-brand)]", hint: "This password has a healthy mix of characters." };
